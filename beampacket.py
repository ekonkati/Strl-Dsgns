...
st.set_page_config(page_title="Head 1 – Design & Shear (Tabs v5)", layout="wide")
...
# tau_c (representative table; replace with your exact values if needed)
...
tab_title, tab_mat, tab_actions, tab_reinf_s, tab_reinf_sp, tab_depth, tab_shear, tab_side, tab_span, tab_summary = st.tabs([
    "Title", "Materials", "Actions", "Reinf. Support", "Reinf. Span",
    "Depth Check", "Shear & Stirrups", "Side-Face", "Span/Depth", "Summary"
])
...
# ---------- DEPTH CHECK ----------
with tab_depth:
    # Limiting depth required to resist both Mu values
    xu = xu_max_ratio(fy)
    C = 0.36 * fck * b * xu * (1 - 0.42 * xu)  # N/mm
...
with tab_shear:
    ...
    if fy_sv == "Use main fy": fy_sv_val = float(fy)
    else: fy_sv_val = float(fy_sv)
...
with tab_span:
    ...
    L_over_d = L / d_span if 'd_span' in locals() and d_span>0 else float('inf')
...
with tab_summary:
    ...
    df = _pd.DataFrame([{"Check": n, "Required": r, "Provided": p, "OK": "Yes" if ok else "No"} for (n,r,p,ok) in items])
