...
def calculate_tau_c(fck, pt):
    ...
    return t0 + (t1 - t0) * (pt - p0) / (p1 - p0)

# --- CSS for Tighter Mobile/Print Layout ---
st.markdown(r"""
<style>
.st-emotion-cache-18ni7ap, .st-emotion-cache-1wb9b6h { padding-left: .5rem!important; padding-right: .5rem!important; }
h1, h2, h3, h4, .stMarkdown, .st-emotion-cache-vdhr9j { font-size: .9rem; line-height: 1.2; }
span, label { font-size: .8rem!important; }
.stSelectbox, .stNumberInput { height: 30px; }
.stSelectbox>div>div, .stNumberInput>div>div>input { min-height: 30px!important; padding: 2px 5px!important; font-size: .8rem!important; }
.st-emotion-cache-p2n2mc, .st-emotion-cache-16ya12x { gap: .5rem; }
@media print {
  .st-emotion-cache-6v09g0, .st-emotion-cache-1avcm0d { display: none!important; }
  .st-emotion-cache-1vq4p4c { max-width: 100%!important; }
...
if tau_v <= tau_c:
    ...
    st.warning(f"**$\\mathbf{{\\tau_v}}$ ({tau_v:.3f}) $>\\mathbf{{\\tau_c}}$ ({tau_c:.3f})**. Shear reinforcement is **required**. $\\mathbf{{V_s}} = \\mathbf{{{Vs_req:.2f} \\text{{ kN}}}}$")
...
st.markdown(f"**Deflection Check Result**: {OK if actual_ld <= permitted_ld else NOT_OK}")
