...
with c_ast1:
    # FIX: Used standard double braces for literal braces in f-string
...
tau_v = Vu * 1000 / (b * d)
with c_s1:
    # FIX: Removed the failing f-string braces and simplified to blue() + standard st.markdown()
    st.markdown(blue(r"Nominal Shear Stress $\tau_v$"))
...
with c_s2:
    # FIX: Removed the failing f-string braces and simplified to blue() + standard st.markdown()
    st.markdown(blue(r"Design Shear Strength $\tau_c$ (Table 19)"))
...
with c_s3:
    # FIX: Removed the failing f-string braces and simplified to blue() + standard st.markdown()
    st.markdown(blue(r"Maximum Shear $\tau_{c, max}$ (Table 20)"))
    st.markdown(f"**{tau_c_max:.1f} N/mm²**")
    
    # Check 1: τv vs τc,max
    result_max_shear = tau_v <= tau_c_max
    # FIX: Explicit $ $ wrapping for the critical comparison - uses simple double braces
...
if tau_v <= tau_c:
    ...
    # FIX: Explicit $ $ wrapping for the critical comparison and result - uses simple double braces
...
with c_d2:
    ...
    # FIX: Four backslashes (\\\\) for \cdot and $ $ wrapping. Uses standard double braces for literal braces
